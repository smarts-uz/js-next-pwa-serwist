...
export function ResourcePrefetchExample() {
  ...
  const allTypesPrefetched = Object.keys(pokemonByType).every(
    (type) => pokemonByType[type as PokemonType].length > 0 && pokemonByType[type as PokemonType][0]?.loaded,
  )
    ...
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-muted-foreground">Pre-fetch Pokémon data before users need it</p>
        </div>
        <Button onClick={prefetchAllTypes} disabled={prefetching || allTypesPrefetched} variant="outline">
          {prefetching ? (
            <>
      ...
      <Tabs value={activeType} onValueChange={(value) => setActiveType(value as PokemonType)} className="w-full">
        <TabsList className="grid w-full grid-cols-3">
          <TabsTrigger value="fire" className="text-red-600">
            Fire
          </TabsTrigger>
          <TabsTrigger value="water" className="text-blue-600">
            Water
          </TabsTrigger>
          <TabsTrigger value="grass" className="text-green-600">
            Grass
          </TabsTrigger>
        </TabsList>
              ...
              {pokemonByType[type].length > 0
                ? pokemonByType[type].map((pokemon) => (
                    <Card key={pokemon.id} className="overflow-hidden">
                      ...
                      <CardFooter className="p-3 pt-0 bg-muted/30 flex justify-center gap-2">
                        {pokemon.types.map((t) => (
                          <span
                            key={t.type.name}
      ...
      <div className="rounded-md bg-muted p-4">
        ...
        <p className="text-sm text-muted-foreground">
          This example demonstrates pre-fetching resources before they're needed using the real PokéAPI. When you click
          "Pre-fetch All Types", the app loads Pokémon data for all types in the background using Web Workers, even
          though you're only viewing one type at a time. This provides instant loading when you switch tabs.
        </p>
        <p className="text-sm text-muted-foreground mt-2">
          In a production application, you would use the Background Fetch API or service workers to download resources
          without impacting the main thread. This approach significantly improves perceived performance and user
          experience.
        </p>
      </div>
    </div>
