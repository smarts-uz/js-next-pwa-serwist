...
export default function ProtocolHandler() {
  ...
  const handleRegister = () => {
    try {
      ...
      setIsRegistered(true)
      alert("Protocol handler registered successfully. You may need to approve it in your browser settings.")
    } catch (error) {
      console.error("Failed to register protocol handler:", error)
      alert(`Failed to register protocol handler: ${error instanceof Error ? error.message : "Unknown error"}`)
    }
  }
  ...
  return (
    <div className="space-y-6">
      {receivedUrl && (
        <Card className="border-green-200 bg-green-50">
          <CardHeader className="pb-2">
            <CardTitle className="text-green-800">Protocol URL Received!</CardTitle>
            <CardDescription className="text-green-700">
              The application successfully handled a custom protocol URL
            </CardDescription>
          </CardHeader>
          <CardContent>
            <p className="font-mono text-sm bg-white p-2 rounded border border-green-200 break-all">{receivedUrl}</p>
          </CardContent>
        </Card>
      )}
      ...
      <div className="space-y-4">
        <div>
          ...
          <p className="text-sm text-muted-foreground mb-4">
            ...
            <code className="font-mono bg-muted px-1 py-0.5 rounded text-xs">web+pwa://</code> URLs. This allows other
            applications to launch this PWA with custom data.
          </p>
        ...
        <div className="pt-4 border-t">
          ...
          <p className="text-sm text-muted-foreground mb-4">
            Test the protocol handler by clicking the link below. Your browser should ask if you want to open this app.
          </p>
      ...
      <Alert>
        ...
        <AlertDescription>
          Protocol handlers require different levels of user interaction across browsers. Some browsers may show a
          permission prompt, while others may silently ignore the registration.
        </AlertDescription>
      </Alert>
    </div>
  )
