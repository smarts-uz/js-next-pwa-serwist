  ...
  return (
    <div className='space-y-4'>
      ...
      <p className='text-muted-foreground'>
        Register a credential using the button below and choose if you would like to authenticate using FaceID, your fingerprint or USB Security Key.
      </p>
      <button onClick={registerCredential} className='btn'>Register Credential</button>
      {credential && (
        <div>
          ...
          <button onClick={removeCredential} className='btn'>Remove Credential</button>
          <button onClick={authenticate} className='btn'>Authenticate</button>
        </div>
      )}
    </div>
  );
